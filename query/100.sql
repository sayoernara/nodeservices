{
    ...
    "goods":{
        ...
        "getPriceByItem" : "SELECT c.tanggal, d.berat, c.harga_per_gram AS harga, c.id_harga_per_gram as id_ppg FROM harga h JOIN harga_per_gram c ON h.id_harga = c.id_harga JOIN berat d ON c.id_berat = d.id_berat WHERE h.id_item = '19' AND d.berat <= 1000 AND c.tanggal = (SELECT MAX(c2.tanggal) FROM harga_per_gram c2 JOIN harga h2 ON c2.id_harga = h2.id_harga WHERE h2.id_item = h.id_item AND c2.id_berat = d.id_berat) ORDER BY d.berat DESC"
    }
}